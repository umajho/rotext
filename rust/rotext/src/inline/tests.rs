...
mod for_fn_advance_until_potential_ref_link_content_ends {
    use crate::inline::{advance_until_potential_ref_link_content_ends, types::Cursor};

    fn test(input: &[u8], expected: Option<()>, expected_cursor: Cursor) {
        let mut cursor = Cursor::new(0);
        let actual = advance_until_potential_ref_link_content_ends(input, &mut cursor);
...
mod for_fn_advance_until_dicexp_will_be_ended {
    ...
    use crate::inline::{advance_until_dicexp_ends, types::Cursor};

    fn test(input: &[u8], expected: Range<usize>, expected_cursor: Cursor) {
        let mut cursor = Cursor::new(0);
        let actual = advance_until_dicexp_ends(input, &mut cursor);
        assert_eq!((expected, expected_cursor), (actual, cursor))
    }
